import { Card, CardTitle, Chip, IconTile, buttonStyles } from "@/components/ui";
...
export function Escalated({ order }: StateProps) {
  ...
  return (
    <StatusLayout
      ...
      chips={<><Chip tone="danger">⚠ Overdue</Chip><Chip tone="warning">● Escalated to a person</Chip></>}
    >
      <Card>
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div className="flex flex-col"><span className="type-body-strong text-ink">{owner.name}</span><span className="type-body-sm text-muted">{owner.email} · replies within 1 business hour</span></div>
        </div>
        ...
        <a className={buttonStyles({ variant: "secondary", full: true })} href={`mailto:${owner.email}`}>Email {owner.name.split(" ")[0]} directly</a>
      </Card>
    </StatusLayout>
  );
}
