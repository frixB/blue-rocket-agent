...
import { Card, CardTitle, Chip, Note, buttonStyles } from "@/components/ui";
...
export function WeekendQueued({ order }: StateProps) {
  ...
  return (
    <StatusLayout
      ...
      chips={<><DeliveryPromise paidAt={order.paidAt} /><Chip icon={Clock}>Queued, starts {start}</Chip></>}
    >
      <Card>
        ...
        <a href={supportHref("I need my report sooner", order.reference)} className={buttonStyles({ variant: "ghost", full: true })}>Need it sooner? Email us</a>
      </Card>
    </StatusLayout>
  );
}
